...
public:
  PlaneExtraction()
      : DrawingAnnotator(__func__), mode("PCL"),
        display(new pcl::PointCloud<pcl::PointXYZRGBA>()), saveToFile(false),
        pointSize(1), cloudname("TestCloud")
  {
  }
  ...
  void getMask(const pcl::PointIndices& inliers, const cv::Size& size,
  ...
  bool process_cloud(pcl::ModelCoefficients::Ptr& plane_coefficients)
  {
    ...
    pcl::PointCloud<pcl::PointXYZRGBA>::Ptr cloud_filtered_no_nan(
        new pcl::PointCloud<pcl::PointXYZRGBA>);

    mapping_indices.clear();

    pcl::removeNaNFromPointCloud(*cloud_filtered, *cloud_filtered_no_nan,
                                 mapping_indices);
    ...
    if (plane_inliers->indices.size() > 0)
    {
      writer.writeASCII("plane.pcd", *cloud_filtered_no_nan,
                        plane_inliers->indices);
    }
